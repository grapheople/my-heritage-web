...
"나의 방" 서비스의 웹 구현체. 취미 수집품을 가상의 방에 전시하고, 남의 방을 들여다보고, 그 물건을 그대로 매물로 전환할 수 있는 컬렉터 커뮤니티.
...
| 2 | `projects/myroom-service/drafts/02-planning-spec.md` | **서비스 골격** — IA, 화면 목록(S-01~S-21), 상태 정의, 데이터 모델(§5), 어드민 화면(§6) |
...
디자인은 디자이너 Figma 1차 탐색 대기 상태다. 프로토타입(HTML 45화면)이 `../my-heritage-planning/projects/myroom-service/prototype/`에 있으니 화면을 구현하기 전에 해당 화면을 확인한다.
...
| 앱 타이틀 ja/en (`app.title`) | 컨셉명 '나의 방'은 브랜딩 레이어. ja/en 브랜드명 미확정 | PM 컨펌 |
| 디자인 (S-01~S-21 전부) | 디자이너 Figma 1차 탐색 대기 | 화면 구현 |
...
기타 미해결 오픈 이슈(OI-17 ~ OI-39)는 `myroom-service/drafts/02-planning-spec.md` "오픈 이슈" 표에 있다.
