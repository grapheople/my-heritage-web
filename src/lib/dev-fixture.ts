 ...
 *
 * ⚠️ **판매자 방 이름이 비로그인·크롤러에게 노출되는 것은 의도된 것이다**
 * (**D-098**, FR-07-A-09). `FR-07-A-08`(소유자 목록 서버 렌더 금지)과 모순이
 * 아니다 — 가르는 기준은 데이터 종류가 아니라 **유저가 공개를 선택했는지**다.
 * 판매자는 스스로 매물로 올렸고, 그 매물은 마켓에서 이미 색인된다.
 * 여기서만 가려도 막히는 게 없다.
 *
 * 판매완료로 전환되면 이 섹션에서 빠진다 (FR-07-B-10).
 */
