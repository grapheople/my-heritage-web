 ...
 *
 * ## ⚠️ 경험치를 주지 않는다
 * 일기는 그날 첫 작성에 20 을 준다 (D-026). 착용샷에도 주면 **같은 날 일기와
 * 착용샷을 둘 다 써서 두 배로 받는다** — 경험치는 보상이 아니라 리듬이라는
 ...
 * 밤 10시에 남긴 착용샷이 "다음 날" 로 기록되면 하루 1장 제약이 이상하게 걸린다.
 */
...
export async function createWearShotAs(
  ...
  try {
    ...
    return fail(
      {},
      "오늘은 이미 착용샷을 남겼어요. 바꾸려면 기존 것을 수정해주세요",
    );
  }
}

/**
 * 착용샷 수정 — 사진·노트만. **날짜는 못 바꾼다.**
 *
...
export async function updateWearShot(input: {
  ...
  const shot = await prisma.wearShot.findFirst({
    // 소유 확인을 아이템의 방으로 한다 — 착용샷에 소유자 컬럼을 따로 두지 않는다
