 ...
 *
 * ## ⚠️ 지금은 **하나다** (D-320)
 * 한때 서울 전용 제공자를 함께 뒀지만, 잔여시간만 주고 상태·좌표는 활용신청이
 * 따로라 **잘 돌 때조차 절반짜리**였고 끝내 백엔드 장애로 모든 요청이 8~40초 뒤
 * 500 이 됐다. 유저는 서울 교차로를 고를 때마다 15초를 기다렸다가 실패를 봤다.
 *
 * ## ⚠️ 그래도 **배열은 남긴다**
 * 하나로 줄었다고 `covers()`·등록부를 걷어내면, 경찰청 TOD 처럼 다음 제공자를
 * 붙일 때 **이 파일을 다시 설계해야 한다.** 지금 남기는 비용은 배열 한 줄이다.
 */
 ...
 * 신호종별 필드가 없다"** 로 옮긴다. 2026-09-12 에 포털이 DB 장애로 500 을 내고
 * 있었는데 화면에는 *"개방 대상이 아닌 교차로"* 라고 떴다 — **원인과 정반대의
 * 안내**다. 유저는 교차로를 계속 바꿔 보게 되고 그때마다 15초를 기다린다.
 */
...
export async function readPhases(
   ...
   *
   * ⚠️ **없으면 전부 부르고, 그 값이 곧 대기 시간이다.** 제공자가 둘이던 시절,
   * 울산 교차로를 물으면서 서울 전용 포털까지 가 15초 타임아웃을 기다렸다 —
   * **9.4초짜리 응답이 0.3초로 끝날 수 있는 자리였다** (2026-09-12 실측). 제공자가
   * 다시 늘면 같은 일이 생기므로 **좁히기를 남겨둔다.** 좌표는
 ...
 *
 * ⚠️ **한 제공자의 실패가 전체를 멈추지 않는다.** 제공자가 둘이던 시절, 한쪽의
 * 교차로 목록이 404(활용신청 미승인)를 내면 그 예외가 올라가 **다른 쪽이 이미 받은
