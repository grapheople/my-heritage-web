...
import { getFollowing } from "@/lib/data/follow";
...
export default async function FollowingPage({
  params,
}: PageProps<"/[locale]/rooms/[roomId]/following">) {
  ...
  const rooms = await getFollowing(room.userId, viewer);

  return (
    <div>
      <header className="border-b px-4 py-4 lg:px-0">
        <h1 className="text-lg font-bold tracking-tight">{t("follow.followingTitle")}</h1>
        <p className="mt-0.5 text-sm text-muted-foreground">{room.name}</p>
      </header>

      {rooms.length === 0 ? (
        <EmptyState title={t("follow.noFollowing")} />
      ) : (
