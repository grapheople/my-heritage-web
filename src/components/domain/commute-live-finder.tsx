...
export function CommuteLiveFinder({
  ...
  function searchByLocation() {
    if (!navigator.geolocation) {
      setMessage(t("findGeoDenied"));
      return;
    }
    setBusy(true);
    setMessage(t("findLocating"));
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // 거부·시간초과를 구분해 봐야 사용자가 할 일은 같다 — 권한 안내 하나로 둔다
        setBusy(false);
        setMessage(t("findGeoDenied"));
      },
    ...
    <section className="rounded-xl border p-4">
      ...
      {open && (
        <div className="mt-2 space-y-3">
          ...
          <Button variant="outline" className="w-full" disabled={busy} onClick={searchByLocation}>
            {t("findByLocation")}
          </Button>
