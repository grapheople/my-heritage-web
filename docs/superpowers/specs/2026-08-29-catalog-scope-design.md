...
- **D-253** 종류를 필수로 — D-207 결정 5 의 전제를 제거한다. 선택지: 필수 / 브랜드만 종류화 / 선택 유지+도감 분할(탈락, D-207 재현)
- **D-254** 스코프 = `COALESCE(subtypeId, categoryId)` 생성 컬럼. 선택지: 생성 컬럼 / XOR 부분 유니크 두 벌 / 전 카테고리 더미 종류
- **D-255** 도감은 배타적 scope, 브랜드는 포함적 scope — 방향이 다른 이유
- **D-256** 자전거 `완성차` 종류 신설. 종류 8개
- **D-257** 캠핑 필수화는 분류 이후 — 폴백 조회로 때우지 않는 이유

---

# 부록 A — 등산 카테고리 신설 + 캠핑 종류 보강 (단계 2 후속)
...
캠핑 208건을 AI로 분류한 결과 **124건 분류 / 84건 unknown**. unknown 을 뜯어보니
**오염보다 "종류가 없어서" 가 두 배 많았다.**

| 귀속 | 건수 | 근거 |
|---|---|---|
| **등산 · 배낭** | 24 | Osprey Atmos 65 · Gregory Baltoro · Deuter Aircontact · Hyperlite · Zpacks — 전부 백패킹 팩 |
| **등산 · 등반장비** | 5 | Mammut 로프·하네스·빌레이·헬멧·비콘 |
| **캠핑 · 조리도구** | 13 | Lodge 더치오븐·스킬렛 · Petromax · 밥통 |
| **캠핑 · 물병** | 9 | Hydro Flask · Stanley |
| **신발**(기존 카테고리) | ~16 | Danner·Columbia 부츠, Merrell·Salomon 러닝화 |
| **옷**(기존 카테고리) | ~3 | Columbia 재킷·팬츠 |
| 개별 판단 | ~14 | 베개·행어·정수필터·보냉재 |

즉 **높은 unknown 은 데이터 오염이 아니라 분류 체계의 공백 신호였다.**

## A-2. ⚠️ 배낭이 두 카테고리에 걸린다 — 카테고리 단위의 D-207

`Item.categoryId` 는 단일 FK 다. 배낭을 캠핑·등산 양쪽 종류로 두면 스코프가
갈려 **같은 배낭이 두 도감**이 된다:

```
Osprey Atmos AG 65
  ├─ 캠핑·배낭 스코프 도감   ← A 유저가 캠핑으로 등록
  └─ 등산·배낭 스코프 도감   ← B 유저가 등산으로 등록
```

D-207 이 종류 단위에서 경고한 실패가 **카테고리 단위에서 재현**된다.

**결정: 배낭은 등산에만 둔다.** 캠핑은 "사이트에 벌여놓는 장비"로 정체성이
선명해진다 — 텐트·타프·침낭·매트·버너·랜턴·체어/테이블·쿨러 + 조리도구·물병.
등산은 "메고 가는 것" — 배낭·등반장비.

데이터가 이 구분을 지지한다: 배낭 24건이 전부 백패킹 팩(Hyperlite·Zpacks 는
스루하이킹 전용 브랜드)이다.
...
key            hiking
라벨            등산 / ハイキング / Hiking
매칭 키          { brand, model }      — 캠핑과 같은 결. 고유 ID 체계가 없다
종류            배낭 · 등반장비          — 2종으로 시작
subtypeRequired false                 — 도감이 붙은 뒤 켠다 (D-257 교훈)
```

**⚠️ 종류를 2개로 시작한다.** 스틱·아이젠·헤드램프는 **지금 도감이 0건**이다.
없는 것에 종류를 만들면 유저 폼에 **빈 선택지**가 뜬다 — D-207 의 "없는 것이
기본" 과 같은 태도다.

**⚠️ `subtypeRequired` 를 켜지 않는다.** 이관 전에는 종류 스코프에 도감이 없어
D-257 과 같은 중복 생성이 일어난다. 이관이 끝난 뒤 켠다.

## A-4. 카테고리 신설이 건드리는 곳

| 대상 | 내용 |
|---|---|
| `prisma/setup-hiking-category.ts` | 카테고리·속성·선택지·조합·매칭 키 (선례 `setup-workout-category.ts`, D-166) |
| `src/lib/categories.ts` | `CATEGORY_KEYS` 에 `hiking` — ⚠️ **코드 배열이라 손으로 고쳐야 한다** (OI-82) |
| `messages/{ko,ja,en}.json` | `category.hiking` — 3개 언어 (D-010) |
| `BrandScope` | Osprey·Gregory·Deuter·Mammut·Hyperlite·Zpacks·Fjällräven 등 |
| 도감 | 캠핑에서 **29건 이관** (배낭 24 + 등반장비 5) |

## A-5. ⚠️ 카테고리를 넘는 도감 이관 — 새 코드가 필요하다

`setCodexSubtype` 은 **같은 카테고리 안에서만** 옮긴다. 캠핑 → 등산은
`categoryId` 가 바뀌므로 함께 움직여야 하는 것이 늘어난다:

- `CodexItem.categoryId` + `subtypeId`
- `CodexMatchKey` 의 두 사본 (`categoryId`·`subtypeId`)
- 옮길 스코프의 유일성 재판정 (`normalizedKey` ∪ 키 alias)
- ⚠️ **연결된 유저 아이템** — `Item.categoryId` 도 바뀌어야 한다

지금 이관 대상 29건의 보유자는 0명이라 마지막 항목이 무해하다. **그래도
스크립트가 확인하고, 보유자가 있으면 멈춘다** — 유저 아이템의 카테고리를
말없이 바꾸면 그 유저의 방 진열이 이동한다.

**범용 카테고리 이동 UI 는 만들지 않는다.** D-216 이 "간단하지 않다"고 판단해
`db:fix-camping-mix` 가 옮기지 않고 지운 그 작업이다. 일회성 스크립트로 둔다.

## A-6. 신발·옷 19건은 옮기지 않고 지운다

Danner·Columbia 부츠와 재킷은 **신발·옷 카테고리 소관**이다. 이동 경로를
만드는 비용이 이득보다 크다 — 두 카테고리에 이미 도감이 85·339건 있어
재수집이 자연스럽다. `db:fix-camping-mix` 와 같은 판단이다.

## A-7. 단계

| 단계 | 내용 | 미분류 |
|---|---|---|
| 1 | 캠핑 종류 `조리도구`·`물병` 추가 → 재분류 → `--apply` | 84 → ~31 |
| 2 | 등산 카테고리 신설 (스크립트 + 코드 배열 + i18n + 브랜드) | — |
| 3 | 캠핑 → 등산 도감 29건 이관 | ~31 → ~2 |
| 4 | 신발·옷 오염 19건 삭제 | — |
| 5 | 남은 개별 판단 어드민 처리 | → 0 |
| 6 | 캠핑·등산 `subtypeRequired` 켜기 | — |

## A-8. 하지 않는 것

| 항목 | 이유 |
|---|---|
| 배낭을 캠핑에도 두기 | 같은 배낭이 두 도감으로 갈린다 (A-2) |
| 등산 종류 4종 이상 | 스틱·아이젠·헤드램프는 도감 0건. 빈 선택지가 뜬다 |
| 범용 카테고리 이동 UI | D-216 판단. 일회성 스크립트로 충분 |
| 신발·옷 19건 이동 | 이동 비용 > 재수집 비용 |
| 캠핑을 "아웃도어" 로 워싱 | PM 이 등산 카테고리를 요청했다. 용어 워싱은 전수 동기화 비용이 따로 든다 |
