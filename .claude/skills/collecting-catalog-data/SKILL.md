...
### ⚠️ 종류가 있으면 **브랜드도 종류로 좁힌다**

자전거 연결 브랜드 40개가 **전부 완성차 브랜드**여서, 부품을 수집하려 하니
**Trek에게 구동계를 묻는 꼴**이었다 (D-263). Shimano·DT Swiss 같은 부품
브랜드는 마스터에 아예 없었다.

부품 브랜드는 **그 종류에만** 연결한다 — 안장 폼에 Shimano가 뜨지 않는다.

⚠️ **브랜드 게이트는 종류를 본다.** `insertCodex`가 `subtypeId`를 받으면
`resolveMasterBrand(name, categoryKey, subtypeKey)`로 넘긴다. 안 넘기면 종류
전용으로 붙인 브랜드가 **전부 거부된다** — 실제로 Dura-Ace 4건이 그렇게 막혔다.

⚠️ **alias가 게이트를 통과시킨다.** 모델이 `Brooks England`로 냈는데 마스터는
`Brooks`라 4건이 막혔다. alias 하나 넣으니 바로 통과했다 (D-047).
...
### ⚠️ 프롬프트에 남은 **옛 전제**를 의심한다

`prompts/codex-research.md`가 **"완성품만 냅니다 — 부품은 후보가 아닙니다"**로
`Shimano Ultegra R8100`을 콕 집어 금지하고 있었다. 근거는 *"부품 도감 체계가
아직 만들어지지 않았습니다"* — 그 전제가 종류 축(D-253) 도입으로 사실이 아니게
됐는데 **프롬프트만 옛 세계에 남아 있었다** (D-264).

**모델은 지시를 정확히 따른다.** 0건이 계속 나오면 프롬프트가 그것을 금지하고
있는지 먼저 본다.
...
### 지울 때 — **보유자와 병합 흡수분을 먼저 본다**

`pnpm tsx prisma/purge-unclassified-codex.ts <카테고리> [--subtype=키] [--apply]`

- `--subtype` 없으면 **미분류분**, 있으면 **그 종류 전체**
- **종류 자체는 지우지 않는다** — 종류가 필수인 카테고리에서 지우면 유저가 그
  종류로 등록할 방법이 사라지고, 자전거는 `parentId` 구성 관계의 부모가 없어진다
- 보유자가 있으면 **멈춘다.** `Item.codexItemId`가 `SetNull`이라 아이템은 살지만
  **도감 연결을 잃고** 명칭이 `brand+model` 파생으로 바뀐다 (D-073).
  `--force-owned` 는 확인한 뒤에만
- 병합 흡수분이 있으면 멈춘다 — 되돌리기가 절반만 되는 상태가 된다 (D-181)

⚠️ **검산 문구가 틀리면 검산이 아니다.** 강제 진행했는데 "보유자 0명 확인"이
찍히거나, 종류 지정 삭제인데 "남은 미분류"라고 나오면 **틀린 안심을 준다**
(D-183과 같은 유형).
...
| 브랜드가 **전부 완성차 브랜드**라 부품을 물을 곳이 없었음 | "종류가 있으면 브랜드도 종류로 좁힌다" (D-263) |
| 브랜드 게이트에 종류를 안 넘겨 전량 거부 | 같은 절 (D-264) |
| **프롬프트가 부품을 금지**하고 있어 0건 | "프롬프트에 남은 옛 전제를 의심한다" (D-264) |
| 강제 삭제했는데 "보유자 0명 확인"이 찍힘 | "검산 문구가 틀리면 검산이 아니다" (D-265) |
...
D-262·D-263·D-264·D-265 의 실제 사고 기록, D-043·D-044·D-047 의 브랜드 규칙이다.
