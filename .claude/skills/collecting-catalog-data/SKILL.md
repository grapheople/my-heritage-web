...
```bash
pnpm db:research-codex <categoryKey> <perBrand> [브랜드명...]
```
...
### ⚠️ `db:research-codex`는 아직 종류를 받지 못한다

인자가 `categoryKey · perBrand · onlyBrand · hint` 넷뿐이다
(`prisma/research-codex.ts:137`). **지금 이 스크립트로 수집하면 무조건 카테고리
스코프에 들어간다.**

종류가 있는 카테고리에서 그대로 돌리면:
- 종류를 고른 유저 아이템이 그 도감과 매칭되지 않는다
- 유저가 같은 제품으로 새 도감을 만든다 = 수집한 것이 무의미해진다

**그래서 지금은:**

| 상황 | 할 것 |
|---|---|
| 종류 없는 카테고리 (시계·신발·옷·데스크테리어) | 그대로 수집한다. `scopeId == categoryId`라 영향 없다 |
| 종류 있는 카테고리 (자전거·캠핑) | **스크립트로 대량 수집하지 않는다.** 화면(A-01 상세 도감 탭)에서 종류를 고르고 넣거나, 스크립트에 종류 인자를 먼저 추가한다 |

D-253~D-257 작업이 끝나면 이 절을 갱신할 것.
